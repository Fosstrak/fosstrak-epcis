 ...
 * BEWARE OF UGLY HACK
 *
 * We had to do some modifications to the EPCglobal-epcis.xsd file:
 *
 ...
 *            <xsd:element
 *                  name="extension" type="epcis:EPCISEventListExtensionType"/>
 ...
 *    </xsd:complexType>
 *
 * This changes at least the EventListType.java class
 * file when compiling the stubs.
 *
 * Background:
 * Axis 1.3 does not support xsd:choice. The stubs created by Axis 1.3 from the
 * original schema files were not able to return more than one Event of each
 * type at once, however the specs clearly say that there can be arbitrarily
 * many. Because Axis2 was not ready for production use at the time of this
 * writing, we had not much choice.
 *
 * Implications:
 * The responses generated by the server stubs are still 100% EPCIS standard
 * compliant. However, the results will always be ordered according to type,
 * not in random order as the standard would allow. Our client, however, will
 * only work with our server, because it relies on the fact that the answers
 * are ordered. Using it with servers that do not order the results may lead
 * to arbitrary behaviour of our client (most likely lost results).
 */
...
public class QueryClient {
     ...
     *
     * @param cal with the Calender-Date
     * @return String
     */
     ...
     *
     * @param eventList of type EventListType
     */
    private void prettyPrintEvents(final EventListType eventList) {
        ...
        if (eventList.getObjectEvent() != null) {
            ...
            for (i = 0; i < eventList.getObjectEvent().length; i++) {
                debugTextArea.append("eventTime:\t"
                        + prettyStringCalendar(eventList.getObjectEvent()[i]
                                .getEventTime()) + "\n");
                debugTextArea.append("recordTime:\t"
                        + prettyStringCalendar(eventList.getObjectEvent()[i]
                                .getRecordTime()) + "\n");
                debugTextArea.append("EPCList:\t");
                for (j = 0; j < eventList.getObjectEvent()[i]
                                .getEpcList().length; j++) {
                    debugTextArea.append(" '"
                ...
                if (eventList.getObjectEvent()[i].getBizLocation() != null) {
                    debugTextArea.append("bizLocation:\t"
                            + eventList.getObjectEvent()[i].getBizLocation()
                                    .getId().toString() + "\n");
                } else {
                ...
                for (j = 0; j < eventList.getObjectEvent()[i].
                    getBizTransactionList().length; j++) {
                    debugTextArea.append("\t'"
                            + eventList.getObjectEvent()[i].
                                    getBizTransactionList()[j].getType()
                            + "', '"
                            + eventList.getObjectEvent()[i]
                                      .getBizTransactionList()[j].getValue()
                                      .toString()
                            + "'\n");
                }
                debugTextArea.append("\n");
        ...
        if (eventList.getAggregationEvent() != null) {
            ...
            for (i = 0; i < eventList.getAggregationEvent().length; i++) {
                debugTextArea.append("eventTime:\t"
                        + prettyStringCalendar(eventList
                                .getAggregationEvent()[i]
                                .getEventTime()) + "\n");
                debugTextArea.append("recordTime:\t"
                        + prettyStringCalendar(eventList
                                .getAggregationEvent()[i]
                ...
                for (j = 0; j < eventList.getAggregationEvent()[i]
                        .getChildEPCs().length; j++) {
                    debugTextArea
                            .append(" '"
                                    + eventList.getAggregationEvent()[i]
                                            .getChildEPCs()[j] + "'");
                }
                debugTextArea.append("\n");
                debugTextArea
                ...
                if (eventList.getAggregationEvent()[i].getReadPoint()
                        != null) {
                    debugTextArea.append("readPoint:\t"
                            + eventList.getAggregationEvent()[i].getReadPoint()
                                    .getId().toString()
                            + "\n");
                } else {
                    debugTextArea.append("readPoint:\tnull\n");
                }
                if (eventList.getAggregationEvent()[i].getBizLocation()
                        != null) {
                    debugTextArea.append("bizLocation:\t"
                            + eventList.getAggregationEvent()[i]
                                    .getBizLocation().getId().toString()
                            + "\n");
                } else {
                    debugTextArea.append("bizLocation:\tnull\n");
                }
        ...
        if (eventList.getQuantityEvent() != null) {
            ...
            for (i = 0; i < eventList.getQuantityEvent().length; i++) {
                debugTextArea.append("eventTime:\t"
                        + prettyStringCalendar(eventList.getQuantityEvent()[i]
                                .getEventTime()) + "\n");
                debugTextArea.append("recordTime:\t"
                        + prettyStringCalendar(eventList.getQuantityEvent()[i]
                                .getRecordTime()) + "\n");
                ...
                debugTextArea.append("disposition:\t"
                        ...
                        + "\n");
                if (eventList.getQuantityEvent()[i].getReadPoint()
                        != null) {
                    debugTextArea.append("readPoint:\t"
                            + eventList.getQuantityEvent()[i].getReadPoint()
                                    .getId().toString()
                            + "\n");
                } else {
                    debugTextArea.append("readPoint:\tnull\n");
                }
                if (eventList.getQuantityEvent()[i].getBizLocation()
                        != null) {
                    debugTextArea.append("bizLocation:\t"
                            + eventList.getQuantityEvent()[i]
                                    .getBizLocation().getId().toString()
                            + "\n");
                } else {
                    debugTextArea.append("bizLocation:\tnull\n");
                ...
                debugTextArea.append("bizTransactions: Type, ID:\n");
                for (j = 0; j < eventList.getQuantityEvent()[i]
                              .getBizTransactionList().length; j++) {
                    debugTextArea.append("\t'"
                            + eventList.getQuantityEvent()[i]
                                    .getBizTransactionList()[j].getType()
            ...
            for (i = 0; i < eventList.getTransactionEvent().length; i++) {
                ...
                debugTextArea.append("recordTime:\t"
                                + prettyStringCalendar(eventList
                                        ...
                                        .getRecordTime()) + "\n");
                debugTextArea.append("parentID:\t"
                        + eventList.getTransactionEvent()[i]
                                .getParentID() + "\n");
                debugTextArea.append("epcList:\t");
                for (j = 0; j < eventList.getTransactionEvent()[i]
                                .getEpcList().length; j++) {
                    debugTextArea.append(" '"
                                    + eventList.getTransactionEvent()[i]
                                            .getEpcList()[j] + "'");
                ...
                debugTextArea.append("disposition:\t"
                        ...
                        + "\n");
                if (eventList.getTransactionEvent()[i].getReadPoint()
                        != null) {
                    debugTextArea.append("readPoint:\t"
                            + eventList.getTransactionEvent()[i].getReadPoint()
                                    .getId().toString()
                            + "\n");
                } else {
                    debugTextArea.append("readPoint:\tnull\n");
                }
                if (eventList.getTransactionEvent()[i].getBizLocation()
                        != null) {
                    debugTextArea.append("bizLocation:\t"
                            + eventList.getTransactionEvent()[i]
                                    .getBizLocation().getId().toString()
     ...
     * is suitable for an JTable object.
     *
     * @param eventList
     * @return
     */
    private Object[][] prettyTableEvents(final EventListType eventList) {
        ...
        if (eventList.getObjectEvent() != null) {
            for (i = 0; i < eventList.getObjectEvent().length; i++) {
                table[row][0] = "Object";
                table[row][1] = prettyStringCalendar(eventList
                        .getObjectEvent()[i].getEventTime());
                table[row][2] = prettyStringCalendar(eventList
                        .getObjectEvent()[i].getRecordTime());
                table[row][5] = "";
                for (j = 0; j < eventList.getObjectEvent()[i]
                        .getEpcList().length; j++) {
                ...
                if (eventList.getObjectEvent()[i].getReadPoint() 
                        != null) {
                    table[row][9] = eventList.getObjectEvent()[i]
                                .getReadPoint().getId().toString();
                }
                if (eventList.getObjectEvent()[i].getBizLocation() 
                        != null) {
                    table[row][10] = eventList.getObjectEvent()[i]
                               .getBizLocation().getId().toString();
                }
                table[row][11] = "";
                for (j = 0; j < eventList.getObjectEvent()[i]
                    ...
                    table[row][11] = table[row][11] + "'"
                        ...
                        + ", " + eventList.getObjectEvent()[i]
                            ...
                            .getValue().toString()
                        + "'";
                    if( eventList.getObjectEvent()[i]
                                   .getBizTransactionList().length > j + 1) {
                        table[row][11] = table[row][11] + "  ;  ";
                    }
                }
                row++;
        ...
        if (eventList.getAggregationEvent() != null) {
            for (i = 0; i < eventList.getAggregationEvent().length; i++) {
                ...
                table[row][1] = prettyStringCalendar(eventList
                        .getAggregationEvent()[i].getEventTime());
                table[row][2] = prettyStringCalendar(eventList
                        .getAggregationEvent()[i].getRecordTime());
                table[row][3] = eventList.getAggregationEvent()[i]
                        .getParentID();
                table[row][5] = "";
                for (j = 0; j < eventList.getAggregationEvent()[i]
                        .getChildEPCs().length; j++) {
                ...
                table[row][7] = eventList.getAggregationEvent()[i]
                         .getBizStep();
                table[row][8] = eventList.getAggregationEvent()[i]
                        .getDisposition();
                if (eventList.getAggregationEvent()[i].getReadPoint() 
                        != null) {
                    table[row][9] = eventList.getAggregationEvent()[i]
                            .getReadPoint().getId().toString();
                }
                if (eventList.getAggregationEvent()[i].getBizLocation() 
                        != null) {
                    table[row][10] = eventList.getAggregationEvent()[i]
                            .getBizLocation().getId().toString();
                }
                table[row][11] = "";
                for (j = 0; j < eventList.getAggregationEvent()[i]
                    ...
                    table[row][11] = table[row][11] + "'"
                        ...
                        + ", " + eventList.getAggregationEvent()[i]
                            ...
                            .getValue().toString()
                        + "'";
                    if( eventList.getAggregationEvent()[i]
                                   .getBizTransactionList().length > j + 1) {
                        table[row][11] = table[row][11] + "  ;  ";
                    }
                }
        ...
        if (eventList.getQuantityEvent() != null) {
            for (i = 0; i < eventList.getQuantityEvent().length; i++) {
                ...
                table[row][1] = prettyStringCalendar(eventList
                        .getQuantityEvent()[i].getEventTime());
                table[row][2] = prettyStringCalendar(eventList
                        .getQuantityEvent()[i].getRecordTime());
                table[row][4] = ""
                        + eventList.getQuantityEvent()[i].getQuantity();
                table[row][5] = "EPC class "
                        + eventList.getQuantityEvent()[i].getEpcClass();
                table[row][7] = eventList.getQuantityEvent()[i].getBizStep();
                table[row][8] = eventList.getQuantityEvent()[i]
                        .getDisposition();
                if (eventList.getQuantityEvent()[i].getReadPoint() 
                        != null) {
                    table[row][9] = eventList.getQuantityEvent()[i].getReadPoint()
                            .getId().toString();
                }
                if (eventList.getQuantityEvent()[i].getBizLocation() 
                        != null) {
                    table[row][10] = eventList.getQuantityEvent()[i]
                            .getBizLocation().getId().toString();
                }
                table[row][11] = "";
                for (j = 0; j < eventList.getQuantityEvent()[i]
                    ...
                    table[row][11] = table[row][11] + "'"
                        ...
                        + ", " + eventList.getQuantityEvent()[i]
                            ...
                            .getValue().toString()
                        + "'";
                    if( eventList.getQuantityEvent()[i]
                                   .getBizTransactionList().length > j + 1) {
                        table[row][11] = table[row][11] + "  ;  ";
                    }
                }
        ...
        if (eventList.getTransactionEvent() != null) {
            for (i = 0; i < eventList.getTransactionEvent().length; i++) {
                ...
                table[row][1] = prettyStringCalendar(eventList
                        .getTransactionEvent()[i].getEventTime());
                table[row][2] = prettyStringCalendar(eventList
                        .getTransactionEvent()[i].getRecordTime());
                table[row][3] = eventList.getTransactionEvent()[i]
                                   .getParentID();
                table[row][5] = "";
                for (j = 0; j < eventList.getTransactionEvent()[i]
                                   .getEpcList().length; j++) {
                ...
                table[row][6] = eventList.getTransactionEvent()[i]
                         .getAction();
                table[row][7] = eventList.getTransactionEvent()[i]
                        .getBizStep();
                table[row][8] = eventList.getTransactionEvent()[i]
                        .getDisposition();
                if (eventList.getTransactionEvent()[i].getReadPoint() 
                        != null) {
                    table[row][9] = eventList.getTransactionEvent()[i]
                            .getReadPoint().getId().toString();
                }
                if (eventList.getTransactionEvent()[i].getBizLocation() 
                        != null) {
                    table[row][10] = eventList.getTransactionEvent()[i]
                            .getBizLocation().getId().toString();
                }
                table[row][11] = "";
                for (j = 0; j < eventList.getTransactionEvent()[i]
                    ...
                    table[row][11] = table[row][11] + "'"
                        ...
                        + ", " + eventList.getTransactionEvent()[i]
                            ...
                            .getValue().toString()
                        + "'";
                    if( eventList.getTransactionEvent()[i]
                                   .getBizTransactionList().length > j + 1) {
                        table[row][11] = table[row][11] + "  ;  ";
                    }
                }
    ...
    public Object[][] runQuery() throws Exception {
        EPCISServiceBindingStub stub = (EPCISServiceBindingStub) service
                .getEPCglobalEPCISService();
     ...
     * @throws Exception
     */
    public void subscribeQuery(Subscribe subcr) throws Exception {
        EPCISServiceBindingStub stub = (EPCISServiceBindingStub) service
            .getEPCglobalEPCISService();
    ...
    public void unsubscribeQuery(String subscriptionID) {
        try {
            ...
            stub = (EPCISServiceBindingStub) service
                .getEPCglobalEPCISService();
     ...
     * @param text wich is comma-seperated
     * @return the array of strings
     */
    public ArrayOfString stringListToArray(String text) {
