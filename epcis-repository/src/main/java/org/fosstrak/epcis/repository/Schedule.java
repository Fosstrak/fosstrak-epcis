...
/**
 * Separate package for Schedule to keep things a bit
 * organized.
 */
...
/**
 * This is a simple schedule which can return a "next scheduled
 * time" after now or a given time. It is meant to be instantiated
 * with a EPCIS QuerySchedule but could be extended to be used
 * otherwise.
 *
...
// Especially in the ...MadeValid() methods.
     ...
     */
    private static final long serialVersionUID = -3312397965542294330L;

    /**
     * The valid second-values. Caveat:
     * Empty means all seconds are valid.
     */
    private TreeSet<Integer> seconds = new TreeSet<Integer>();
    /**
     * The valid minute-values. Caveat:
     * Empty means all minutes are valid.
     */
    private TreeSet<Integer> minutes = new TreeSet<Integer>();
    /**
     * The valid hour-values. Cave:
     * Empty means all hours are valid.
     */
    private TreeSet<Integer> hours = new TreeSet<Integer>();
    /**
     * The valid days of month-values.
     * Cave: Empty means all days are valid.
     */
    private TreeSet<Integer> daysOfMonth = new TreeSet<Integer>();
    /**
     * The valid month-values. Cave:
     * Empty means all months are valid.
     */
    private TreeSet<Integer> months = new TreeSet<Integer>();
    /**
     * The valid days of week-values.
     * Cave: Empty means all days of week
     * are valid.
     */
    ...
    /**
     * Constructor for creating a new schedule according
     * to the parameters in the given QuerySchedule 'schedule'.
     * @param schedule The EPCIS style schedule to be used
     * for constructing this schedule.
     * @throws SubscriptionControlsException If invalid data
    ...
    public Schedule(final QuerySchedule schedule)
     throws SubscriptionControlsException {
        ...
        // the schedule values.
        if (schedule.getSecond() == null) { schedule.setSecond(""); }
        if (schedule.getMinute() == null) { schedule.setMinute(""); }
        if (schedule.getHour() == null) { schedule.setHour(""); }
        if (schedule.getDayOfMonth() == null) { schedule.setDayOfMonth(""); }
        if (schedule.getMonth() == null) { schedule.setMonth(""); }
        if (schedule.getDayOfWeek() == null) { schedule.setDayOfWeek(""); }
        ...
        if (!months.isEmpty()
                ...
                && months.first() == 1
                && (daysOfMonth.first() ==  30 || daysOfMonth.first() == 31)
                )) {
     ...
     * @throws ImplementationException Almost any kind of error.
     */
    public GregorianCalendar nextScheduledTime()
     ...
     * @param time Time after which next scheduled time should
     * be returned.
     * @return The next scheduled time after 'time'.
     * @throws ImplementationException Almost any kind of error.
     */
    public GregorianCalendar nextScheduledTime(
            final GregorianCalendar time) throws ImplementationException {
        /* Algorithm idea:
         * Start with biggest time unit (i.e. year) of the
         * given time. If unit value is specified to be valid
         * (also implicitly by omitting the value) in the
         * schedule *and* there exists a valid smaller unit for
         * that time unit -> choose it as return value. Do so recursively
         * for all time units. Unfortunately month has to be special
         * cased because of 'dayOfWeek'.
         * Attention: This is an ad hoc idea. If you know a
         * better algorithm speak up and/or implement it. Arthur.
         */
        GregorianCalendar nextSchedule =
            (GregorianCalendar) time.clone();
        // Look at year
    ...
    /**
     * Returns true if the month and all smaller units
     * have been succesfully set to valid values within
     * the set year.
     * @param nextSchedule The current candidate for the
     * result.
     * @return True if month and smaller units succesfully
     * set to valid values.
     * @throws ImplementationException Almost any kind of error.
     */
    private boolean monthMadeValid(GregorianCalendar nextSchedule)
     throws ImplementationException {
        if (!months.contains(nextSchedule.get(MONTH))
                && !months.isEmpty()) {
    ...
    /**
     * Returns true if the day and all smaller units
     * have been succesfully set to valid values within
     * the set month.
     * @param nextSchedule The current candidate for the
     * result.
     * @return True if day and smaller units succesfully
     * set to valid values.
     * @throws ImplementationException Almost any kind of error.
     */
    private boolean dayMadeValid(GregorianCalendar nextSchedule)
     throws ImplementationException {
        if (!daysOfMonth.contains(nextSchedule.get(
          DAY_OF_MONTH))
          && !daysOfMonth.isEmpty()) {
            if (!setFieldToNextValidRoll(
               nextSchedule, DAY_OF_MONTH, HOUR_OF_DAY)) {
                return false;
            }
        }
        ...
        while (!daysOfWeek.contains(nextSchedule.get(DAY_OF_WEEK))
            ...
            if (!setFieldToNextValidRoll(
                nextSchedule, DAY_OF_MONTH, HOUR_OF_DAY)) {
                    return false;
                } else if (!daysOfWeek.contains(
                        nextSchedule.get(DAY_OF_WEEK))) {
    ...
    /**
     * Returns true if the hour and all smaller units
     * have been succesfully set to valid values within
     * the set day.
     * @param nextSchedule The current candidate for the
     * result.
     * @return True if hour and smaller units succesfully
     * set to valid values.
     * @throws ImplementationException Almost any error.
     */
    private boolean hourMadeValid(GregorianCalendar nextSchedule)
     throws ImplementationException {
        if (!hours.contains(nextSchedule.get(HOUR_OF_DAY))
                && !hours.isEmpty()) {
            if (!setFieldToNextValidRoll(
                    nextSchedule, HOUR_OF_DAY, MINUTE)) {
                return false;
            }
        }
    ...
    /**
     * Returns true if the minute and all smaller units
     * have been succesfully set to valid values within
     * the set hour.
     * @param nextSchedule The current candidate for the
     * result.
     * @return True if minute and smaller units succesfully
     * set to valid values.
     * @throws ImplementationException Almost any error.
     */
    private boolean minuteMadeValid(GregorianCalendar nextSchedule)
     throws ImplementationException {
        if (!minutes.contains(nextSchedule.get(MINUTE))
                && !minutes.isEmpty()) {
        ...
        while (!secondMadeValid(nextSchedule)) {
            ...
            // Reset all smaller units to min.
            if (
             !setFieldToMinimum(nextSchedule, SECOND)) {
                return false;
            }
        }
        return true;
     ...
     * @param nextSchedule The current candidate for the
     * result.
     * @return True if second succesfully
     * set to valid values.
     * @throws ImplementationException Almost any error.
     */
    private boolean secondMadeValid(GregorianCalendar nextSchedule)
     throws ImplementationException {
        if (!seconds.contains(nextSchedule.get(SECOND))
                && !seconds.isEmpty()) {
    ...
    /**
     * Sets the field of a GregorianCalender to its next valid value,
     * is defined as the next possible value according to the
     * calendar type used possibly superseded by the defined values
     * in the schedule we have. Returns whether the new value has been
     * set and is valid.
     * @param cal Calendar to adjust.
     * @param field Field to adjust.
     * @return Returns whether the new value has been set and is valid.
     * @throws ImplementationException Almost any error.
     */
    private Boolean setFieldToNextValid(
            GregorianCalendar cal, final int field)
     throws ImplementationException {
        Integer next;
    ...
    /**
     * Sets the field of a GregorianCalender to its next valid value,
     * but first sets all smaller fields to their minima and rolls
     * the datefield
     * is defined as the next possible value according to the
     * calendar type used possibly superseded by the defined values
     * in the schedule we have. Returns whether the new value has been
     ...
     * @param smallerField Field from where on to minimize.
     * @return Returns whether the new value has been set and is valid.
     * @throws ImplementationException Almost any error.
     */
    private Boolean setFieldToNextValidRoll(
            GregorianCalendar cal, final int field, final int smallerField)
     throws ImplementationException {
        cal.add(field, 1);
    ...
    /**
     * Sets the field of a GregorianCalender to its minimum, which
     * is defined as the minimal possible value according to the
     * calendar type used possibly superseded by the defined values
     * in the schedule we have. Returns whether the new value has been
     * set and is valid.
     * @param cal Calendar to adjust.
     ...
     * @throws ImplementationException Almost any error.
     */
    private Boolean setFieldToMinimum(
            GregorianCalendar cal, final int field)
     throws ImplementationException {
        Integer min;
        TreeSet<Integer> vals = getVals(field);
        if (vals.isEmpty()) {
            ...
            if (min > cal.getActualMaximum(field)) {
                ...
                if (!vals.contains(min)
                        || min < cal.getActualMinimum(field)
                        || min > cal.getActualMaximum(field)) {
                 //System.out.println("Rarely we should see an invalid min.");
    ...
    /**
     * Sets the field of a GregorianCalender and all smaller fields
     * (not WEEK_OF_DAY) to its minimum, which
     * is defined as the minimal possible value according to the
     * calendar type used possibly superseded by the defined values
     * in the schedule we have. Returns whether the new values have been
     * set and are all valid.
     * @param cal Calendar instance to adjust.
     * @param largestField This field and smaller ones are reset
     * @return True if setting to min worked for all values.
     * @throws ImplementationException Various errors.
     */
    private Boolean setFieldsToMinimum(
            GregorianCalendar cal, final int largestField)
            throws ImplementationException {
        Boolean result = true;
        switch(largestField) {
            case(MONTH):
    ...
    /**
     * Returns the numbers belonging to the 'field' value
     * of a GregorianCalendar.
     * @param field Field number of GregorianCalendar.
     * @return The schedule numbers belonging to this 'field'.
     * @throws ImplementationException In case of a access to
     * an unknown field.
     */
    ...
    /**
     * Checks whether the 'values', which are either numbers or
     * ranges, are parseable and puts them into the return value.
     * @param values The numbers and ranges to be checked.
     * @throws SubscriptionControlsException In case there's some
     * invalid data.
     * @return Returns an array where the first element is an array
     * of ints with the number values and the second element is an
     * array of arrays which denote the ranges.
     */
    private Object[] parseValuesAndRanges(
            final String[] values)
     throws SubscriptionControlsException {
    ...
    /**
     * Checks whether all ranges lie within min and max and whether
     * range[0] <= range[1].
     * @param rangesArray Ranges to check.
     * @param min Minimal allowed value.
     * @param max Maximal allowed value.
     * @throws SubscriptionControlsException If illegal range encountered.
     */
    private void checkRanges(
            final Integer[][] rangesArray, final int min, final int max)
     throws SubscriptionControlsException {
     ...
     * @param numsArray Numbers to check.
     * @param min Minimal allowed value.
     * @param max Maximal allowed value.
     * @throws SubscriptionControlsException If illegal range encountered.
     */
    private void checkNums(
            final Integer[] numsArray, final int min, final int max)
        throws SubscriptionControlsException {
     ...
     * @param ranges Ranges to add to set.
     * @param set Set to be enhanced with the values of the ranges.
     */
    private void addRangesToTreeSets(
      Integer[][] ranges, TreeSet<Integer> set) {
     ...
     * @param array Array to add to set.
     * @param set Set to be enhanced with the values of the array.
     */
    private void addValuesToTreeSets(
      Integer[] array, TreeSet<Integer> set) {
        for (Integer v : array) {
            set.add(v);
        }
    }

    public void addDayOfMonthRanges(Integer[][] dayOfMonthRanges)
     throws SubscriptionControlsException {
        checkRanges(dayOfMonthRanges, 1, 31);
        addRangesToTreeSets(dayOfMonthRanges, daysOfMonth);
    }

    public void addDayOfMonthValues(Integer[] dayOfMonthValues)
     throws SubscriptionControlsException {
        checkNums(dayOfMonthValues, 1, 31);
        addValuesToTreeSets(dayOfMonthValues, daysOfMonth);
    }

    public void addDayOfWeekRanges(Integer[][] dayOfWeekRanges)
     throws SubscriptionControlsException {
    ...
    public void addDayOfWeekValues(Integer[] dayOfWeekValues)
     throws SubscriptionControlsException {
    ...
    public void addMonthValues(Integer[] monthValues)
     throws SubscriptionControlsException {
    ...
    public void addSecondRanges(Integer[][] secondRanges)
     throws SubscriptionControlsException {
        checkRanges(secondRanges, 0, 59);
        addRangesToTreeSets(secondRanges, seconds);
    }

    public void addSecondValues(Integer[] secondValues)
     throws SubscriptionControlsException {
